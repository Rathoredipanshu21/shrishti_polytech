    ...
    if (isset($_POST['add_product'])) {
        ...
        if (isset($_FILES['product_images'])) {
            ...
            for ($i = 0; $i < $totalFiles; $i++) {
                ...
                $dbPath = "uploads/products/" . $newFileName; // Path for DB
...
<body class="overflow-x-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            if ($res->num_rows > 0):
                while ($row = $res->fetch_assoc()): 
                    $images = json_decode($row['images'], true);
                    $thumb = !empty($images) ? "../" . $images[0] : "https://via.placeholder.com/300?text=No+Image";
            ?>
                <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 group" data-aos="fade-up">
                    <div class="relative h-48 overflow-hidden bg-gray-100">
                        ...
                        <div class="absolute top-2 right-2 bg-black/60 text-white px-2 py-1 rounded text-xs">
                            <i class="fa-solid fa-images"></i> <?php echo count($images); ?>
                        </div>
                    </div>
                    <div class="p-5">
                        ...
                        <div class="border-t pt-4 flex justify-between items-center">
                            ...
                            <form method="POST" onsubmit="return confirm('Delete this product?');">
                                ...
                                <button class="text-[#D71920] hover:text-red-700 font-medium text-sm"><i class="fa-solid fa-trash"></i> Delete</button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <div id="productModal" class="modal fixed inset-0 z-50 items-center justify-center p-4">
        <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden" data-aos="zoom-in">
            <div class="bg-gray-900 px-6 py-4 flex justify-between items-center border-b border-[#D71920]">
